...
public class SqlServerChangeTrackingOperationsTests : IAsyncLifetime
{
    ...
    {
        //// Arrange
        //await DisableDatabaseChangeTracking();
        //await _operations.DisableTracking(_testTableName);
    ...
    {
        //// Arrange - Table already has primary key
        //await DisableChangeTrackingForTable(_testTableName);
    ...
    public async Task EnableTracking_WhenTableHasNoPrimaryKey_ThrowsException()
    {
        ...
        await CreateTableWithoutPrimaryKey(tableNoPk);
    ...
    public async Task EnableTracking_WhenTableHasExistingData_TrackingStartsFromCurrentState()
    {
        ...
        // Insert some data before enabling tracking
        using var connection = new SqlConnection(_connectionString);
        await connection.OpenAsync();
        var insertQuery = $"INSERT INTO [{_testTableName}] (Name) VALUES ('BeforeTracking1'), ('BeforeTracking2')";
    ...
    public async Task EnableTracking_WhenUserLacksAlterPermission_ThrowsException()
    {
        ...
        var ex = await Assert.ThrowsAsync<SqlException>(async () =>
            await _lowPrivilageOperations.EnableTracking(_testTableName));
    ...
    private async Task CreateTableWithoutPrimaryKey(string tableName)
    {
        using var connection = new SqlConnection(_connectionString);
        await connection.OpenAsync();
    ...
    private async Task DropTable(string tableName)
    {
        ...
        var dropQuery = $"DROP TABLE IF EXISTS [{tableName.Replace("]", "]]")}]";
